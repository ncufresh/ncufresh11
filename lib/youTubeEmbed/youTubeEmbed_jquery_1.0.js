(function($){
    ...
    $.fn.youTubeEmbed = function(settings){
        ...
        $.get(youtubeAPI,{'q':settings.videoID},function(response){
            ...
            */
            elements.container = $('<div class="flashContainer"></div>').appendTo(elements.originalDIV) ;
            $('.flashContainer').css({
                'width': settings.width,
                'height': settings.height
            }) ;
            ...
            elements.control = $('<div class="controlDiv play"></div>').appendTo(elements.container) ;
            ...
            if(settings.progressBar){
                ...
                */
                elements.progress = $('<div class="progressBar"></div>').appendTo(elements.container) ;
                elements.elapsed = $('<div class="elapsed"></div>').appendTo(elements.progress) ;
            ...
            window['eventListener_'+settings.safeID] = function(status){
                ...
                if(status==-1)  // video is loaded
                {
                    if(!initialized)
                    {
                        elements.container.hover(function(){elements.control.show();}, 
                            function(){elements.control.hide();}) ;
                        ...
                        elements.control.click(function(){
                            if(!elements.container.hasClass('playing')){
                                ...
                                //alert($('.controlDiv').attr('class')) ;
                                elements.control.removeClass('play replay').addClass('pause');
                                elements.container.addClass('playing');
                                elements.control.css('background-position', 'center center') ;
